...
const nextHead = state => state.snake.length == 0 ? {x: 2,y: 2} : {x: mod(state.columns)(state.snake[0].x + state.moves[0].x),y: mod(state.rows)(state.snake[0].y + state.moves[0].y)};
...
const rndPos = table => ({x: rnd(0)(table.columns - 1),y: rnd(0)(table.rows - 1)});
const initialState = () => ({columns: 20,rows: 14,moves: [EAST],snake: [],apple: {x: 16,y: 2},});
const next = spec({rows: prop('rows'),columns: prop('columns'),moves: nextMoves,snake: nextSnake,apple: nextApple});
